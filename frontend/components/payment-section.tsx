...
export default function PaymentSection() {
  ...
  async function load() {
    try {
      ...
      const [paymentData, invoiceData, workspace] = await Promise.all([api.payments.list(), api.invoices.list(), api.workspace.me()]);
      setPayments(paymentData);
      setInvoices(invoiceData.filter((invoice) => ["sent", "overdue"].includes(invoice.status)));
  ...
  const paidForSelectedInvoice = selectedInvoice
    ? payments
        .filter((payment) => payment.invoice_id === selectedInvoice.id)
        .reduce((sum, payment) => sum + Number(payment.amount), 0)
    : 0;
  const remainingBalance = selectedInvoice
    ? Math.max(0, Number(selectedInvoice.total) - paidForSelectedInvoice)
    : 0;
  const paymentTooHigh = amount > remainingBalance;

  function openCreate() {
    const available = invoices.filter((invoice) => invoice.status !== "paid");
    setInvoiceId(available[0]?.id ?? "");
    setAmount(0);
  ...
  async function save(event: React.FormEvent) {
    ...
    if (!invoiceId || amount <= 0 || paymentTooHigh || saving) return;
    setSaving(true);
    setError(null);
    try {
      const api = await getApi();
      ...
      setInvoices((current) => current.filter((invoice) => invoice.id !== invoiceId || amount < remainingBalance));
      setOpen(false);
    } catch (cause) {

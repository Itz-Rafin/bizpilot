...
import { createApi, type Expense, type Invoice, type Product, type ReportSummary, type Service } from "@/lib/api/client";
...
export default function SectionPage() {
  ...
  const [rows, setRows] = useState<Array<Invoice | Product | Service | Expense>>([]);
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-8 flex items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-[#687485]">A focused view of your business data.</p>
          </div>
          {section !== "settings" && section !== "reports" && (
            <button className="btn-primary">Add {title.slice(0, -1)}</button>
          )}
        </div>
        ...
        {section === "reports" && report ? (
          <div className="mt-8 grid gap-5 sm:grid-cols-3">
            ...
            <div className="card mt-1 p-5 sm:col-span-3">
              <h2 className="font-semibold">Customer revenue</h2>
              <div className="mt-4 space-y-3">
                {report.customer_revenue.length === 0 ? <p className="text-sm text-[#8993a2]">No paid revenue in this period yet.</p> : report.customer_revenue.map((item) => (
                  <div key={item.customer} className="flex justify-between border-b border-[#eef1f5] pb-3 text-sm"><span>{item.customer}</span><span className="font-semibold">${Number(item.revenue).toLocaleString()}</span></div>
                ))}
              </div>
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[620px] text-left text-sm">
                <thead className="border-b border-[#e7ebf0] bg-[#fbfcfe] text-xs uppercase tracking-wider text-[#8993a2]">
                  <tr><th className="px-5 py-4">Name / description</th><th className="px-5 py-4">Status / method</th><th className="px-5 py-4 text-right">Amount / date</th></tr>
                </thead>
                <tbody className="divide-y divide-[#eef1f5]">
                  {rows.length === 0 ? (
                    <tr><td colSpan={3} className="px-5 py-16 text-center"><div className="text-sm font-semibold">No records yet</div><p className="mt-1 text-xs text-[#8993a2]">Use the action above to add your first {title.toLowerCase().replace(/s$/, "")}.</p></td></tr>
                  ) : rows.map((row) => {
                    const item = row as Invoice & Product & Service & Expense;
                    return <tr key={item.id} className="table-row"><td className="px-5 py-4 font-medium">{item.name ?? item.description ?? item.invoice_number}<div className="text-xs font-normal text-[#8993a2]">{item.email ?? item.sku ?? item.company ?? ""}</div></td><td className="px-5 py-4 capitalize text-[#687485]">{item.status ?? item.payment_method ?? "—"}</td><td className="px-5 py-4 text-right font-semibold">{item.total ? `$${Number(item.total).toLocaleString()}` : item.amount ? `$${Number(item.amount).toLocaleString()}` : item.price ? `$${Number(item.price).toLocaleString()}` : item.created_at?.slice(0, 10)}</td></tr>;
                  })}
                </tbody>
              </table>
            </div>
